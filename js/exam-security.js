...
function initExamSecurity(exam, questions) {
    ...
    // Enable randomization -- shuffle question order AND options within each question
    examSecurity.randomizedQuestions = shuffleArray([...examSecurity.originalQuestionOrder]).map(q => {
        if (!q.options || q.options.length <= 2) return q;
        const validIndices = q.options.map((opt, i) => i).filter(i => q.options[i] && q.options[i].trim() !== '' && q.options[i].trim().toUpperCase() !== 'N/A');
        if (validIndices.length <= 2) return q;
        return { ...q, shuffledOptionIndices: shuffleArray([...validIndices]) };
    });
