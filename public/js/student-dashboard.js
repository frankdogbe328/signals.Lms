...
function renderQuestionWithNavigation(exam, contentDiv) {
    ...
    if (questionType === 'essay') {
        ...
        // Filter out "N/A" options for display (but keep them for answer mapping)
        const displayOptions = question.options.filter(opt => opt && opt.trim() !== '' && opt.trim().toUpperCase() !== 'N/A');
        const optionMap = []; // Map display index to original index
        question.options.forEach((opt, idx) => {
            if (opt && opt.trim() !== '' && opt.trim().toUpperCase() !== 'N/A') {
                optionMap.push(idx);
            }
        });
